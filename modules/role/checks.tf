...
check "policy_config" {
  assert {
    ...
    error_message = "Either 'read_only_policy_arns' or 'read_only_inline_policies' must be specified"
  }

  assert {
    condition     = !(length(var.read_write_policy_arns) == 0 && length(var.read_write_inline_policies) == 0)
    error_message = "Either 'read_write_policy_arns' or 'read_write_inline_policies' must be specified"
  }
}
