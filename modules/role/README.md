...
### **Restricting Where a Role Can Be Assumed From**

The trust policies generated by this module validate the OIDC `audience` and `subject` claims. Any additional IAM condition — a network boundary, an extra OIDC claim, a time window — is supplied through `trust_policy_conditions`. Each entry is appended verbatim as a `condition` block to the trust policy of **every** role the module creates (read-write, read-only and state reader), including the Azure DevOps hub-to-spoke `sts:AssumeRole` statements.
...
module "terraform_roles" {
  ...
  trust_policy_conditions = [
    # Only permit the roles to be assumed from a specific VPC
    {
      test     = "StringEquals"
      variable = "aws:SourceVpc"
      values   = ["vpc-0123456789abcdef0"]
    },
    # Only permit a specific GitHub Actions actor
    {
      test     = "StringEquals"
      variable = "token.actions.githubusercontent.com:actor"
      values   = ["github-actions[bot]"]
    },
  ]
...
> [!IMPORTANT]
> `aws:SourceVpc`, `aws:SourceVpce` and `aws:VpcSourceIp` are only present in the request context when the call reaches AWS through an **interface VPC endpoint**. Because this condition sits on the trust policy, the `sts:AssumeRoleWithWebIdentity` call itself must traverse an STS interface endpoint — i.e. the runner has to be **self-hosted inside the VPC**. GitHub/GitLab-hosted (SaaS) runners call the public STS endpoint, the key is absent, and the condition fails closed, making the role unassumable.

Conditions targeting a `:aud` or `:sub` claim are rejected by validation — the module already emits those keys, and a duplicate would silently override the repository scoping. Use `additional_audiences`, `repository`/`repositories` and `protected_by` instead. Note also that these conditions govern role *assumption* only; to constrain the API calls made with the resulting credentials, use `default_inline_policies` or `permission_boundary_arn`.
